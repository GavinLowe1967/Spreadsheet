...
The application uses the Scala Swing library, so that needs to be on the
...
|three| into cell |#A0|, the error panel displays the following error.
%
\begin{trivlist}
\item[]
\footnotesize
...
represent error.  The error panel indicates that the cell~|#A0| read at
...
the error panel also explains an error a type error corresponding to the write
...
provided input, \emph{output cells}, written to by the script, and \emph{empty
  cells}.  Any attempt to write to an output cell is considered an error and
