...
//   private def typeCheckBinOp(typeEnv: TypeEnv, left: Exp, op: String, right: Exp)
//       : Reply[TypeCheckRes] =
...
//       typeCheckBinOp(typeEnv, left, op, right).lift(exp)
...
//         }).lift(stmt)
  ...
  private def typeCheckStmtList(typeEnv: TypeEnv, stmts: List[Statement])
      : Reply[TypeEnv] = {
    checkDisjointNames(typeEnv, stmts).map{ iterTypeCheckStmts(_, stmts) }
...
// IMPROVE: remove common code with iterTypeCheckDecls
